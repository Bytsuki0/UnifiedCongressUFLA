...
export const Route = createFileRoute("/_authenticated/minicursos")({
  head: () => ({ meta: [{ title: "Minicursos — Congresso UFLA Paraíso" }] }),
...
function Minicursos() {
  ...
  const list = useQuery({
    ...
    queryFn: async () => (await supabase.from("minicourses").select("*").order("data").order("horario_inicio")).data ?? [],
  });
  const myRegs = useQuery({
    queryKey: ["my-mini-regs", uid],
    queryFn: async () => (await supabase.from("minicourse_registrations").select("minicourse_id").eq("user_id", uid)).data ?? [],
  });
  const counts = useQuery({
    queryKey: ["mini-counts"],
    queryFn: async () => {
      const { data } = await supabase.from("minicourse_registrations").select("minicourse_id");
  ...
  const subscribe = useMutation({
    mutationFn: async (minicourse_id: string) => {
      const { error } = await supabase.from("minicourse_registrations").insert({ user_id: uid, minicourse_id });
