...
export const Route = createFileRoute("/_authenticated/dashboard")({
  head: () => ({ meta: [{ title: "Dashboard — Congresso UFLA Paraíso" }] }),
  component: Dashboard,
});
...
function Dashboard() {
  ...
  const certs = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("certificates").select("*").eq("user_id", uid).not("data_liberacao", "is", null)).data ?? [],
  });
  const next = useQuery({
    queryKey: ["next-schedule"],
    queryFn: async () =>
      (await supabase.from("schedule").select("*").order("data").order("horario_inicio").limit(3)).data ?? [],
  });
    ...
    <AppLayout>
      <div className="flex flex-col gap-6">
        ...
        <Section title="Próximas atividades">
          ...
          <Link to="/programacao" className="self-end text-sm text-primary hover:underline">
            Ver tudo <ChevronRight className="inline h-3 w-3" />
          </Link>
        </Section>

        <Section title="Meus minicursos">
          {minis.data?.length === 0 && <Empty>Você ainda não inscreveu em minicursos. <Link to="/minicursos" className="text-primary hover:underline">Ver minicursos</Link></Empty>}
          {minis.data?.map((m) => (
